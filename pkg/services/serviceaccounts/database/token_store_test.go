...
func TestIntegrationStore_UpdateAPIKeysExpiryDate(t *testing.T) {
	...
	keyName := t.Name()
	key, err := apikeygen.New(sa.OrgID, keyName)
	require.NoError(t, err, "An error has ocurred when greating a service account token")
	...
	require.NoError(t, err, "An error has ocurred when adding an access token to  a service account")
	err = store.UpdateAPIKeysExpiryDate(context.Background(), 7)
	require.NoError(t, err, "An error has ocurred when updating the Expiry dates")

	tokens, err := store.ListTokens(context.Background(), &serviceaccounts.GetSATokensQuery{})
	require.NoError(t, err, "An error has ocurred when listing tokens")
